  ...
  try {
    ...
    const findTeachers = (identifier: string): { found: any[]; missing: string[] } => {
      ...
      const foundIds = new Set(
        found.map(t => t?.correoInstitucional || t?.correoPersonal || t?.document)
      );
    ...
    // Process subjects in batches with transaction
    const SUBJECT_BATCH_SIZE = 50;
    const subjectEntries = Array.from(subjectsMap.entries());

    for (let i = 0; i < subjectEntries.length; i += SUBJECT_BATCH_SIZE) {
      const batch = subjectEntries.slice(i, i + SUBJECT_BATCH_SIZE);

      await db.$transaction(async tx => {
        for (const [uniqueKey, data] of batch) {
          ...
          try {
            ...
            // Prepare classes for bulk insert
            const classesToCreate = data.classes
              .filter((clsRow: RowData) => {
                ...
                return dateRaw && startRaw && endRaw;
              })
              .map((clsRow: RowData) => {
                const dateRaw = clsRow[keys.date as keyof RowData] as string | number | Date;
                const startRaw = clsRow[keys.start as keyof RowData] as string | number;
                const endRaw = clsRow[keys.end as keyof RowData] as string | number;
                ...
                return {
                  ...
                  classroom:
                    String(clsRow[keys.classroom as keyof RowData] || '') || subject.classroom,
                };
              });

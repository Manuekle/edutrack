  ...
  try {
    ...
    if (!file || !subjectIdsJson) {
      return NextResponse.json(
        { error: 'Faltan datos requeridos (archivo o asignaturas)' },
        { status: 400 }
      );
    }
    ...
    const docIndex = headers.findIndex(
      h => h.includes('documento') || h.includes('identificacion')
    );
    const emailIndex = headers.findIndex(h => h.includes('correo') || h.includes('email'));

    if (docIndex === -1 && emailIndex === -1) {
      return NextResponse.json(
        { error: 'El archivo debe tener una columna "documento" o "correo"' },
        { status: 400 }
      );
    }
